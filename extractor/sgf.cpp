...
string SgfNode::getSinglePropertyOrDefault(const std::string& key, const std::string& defaultValue) const {
  if(hasProperty(key))
    return getSingleProperty(key);
  else
    return defaultValue;
}
...
float Sgf::overtimeSecondsPerMove() const {
  checkNonEmpty(nodes);
  if(!nodes[0]->hasProperty("OT"))
    return -1;
  string overtime = nodes[0]->getSingleProperty("OT");

  static const std::regex canadian_pattern("(\\d+)/(\\d+) Canadian");
  static const std::regex fischer_pattern("(\\d+) fischer");
  static const std::regex byoyomi_pattern("(\\d+)x(\\d+) byo-yomi");
  static const std::regex simple_pattern("(\\d+) simple");

  std::smatch match;
  if(std::regex_search(overtime, match, canadian_pattern)) {
    float stones = float(Global::parseDigits(match[1]));
    float time = float(Global::parseDigits(match[2]));
    return time/stones;
  }
  if(std::regex_search(overtime, match, fischer_pattern)) {
    float time_increment = float(Global::parseDigits(match[1]));
    return time_increment;
  }
  if(std::regex_search(overtime, match, byoyomi_pattern)) {
    float periods = float(Global::parseDigits(match[1]));
    float time = float(Global::parseDigits(match[2]));
    return time;
  }
  if(std::regex_search(overtime, match, simple_pattern)) {
    float time = float(Global::parseDigits(match[1]));
    return time;
  }
  else {
    return -1;
